...
public class SymbolRenderer extends ElementRenderer<Symbol> {
    ...
    @Override
    public void render(ImDrawList drawList, ImVec2 offset, Element element, BoundingBox bb, Symbol data) {
        float x = offset.x + bb.x(), y = offset.y + bb.y();
        int color = data.color().asABGR();
        switch (data.type()) {
        case ARROW -> {
            drawList.addQuadFilled(QUAD_1[0].x * bb.width() + x, QUAD_1[0].y * bb.height() + y,
                QUAD_1[1].x * bb.width() + x, QUAD_1[1].y * bb.height() + y,
                QUAD_1[2].x * bb.width() + x, QUAD_1[2].y * bb.height() + y,
                QUAD_1[3].x * bb.width() + x, QUAD_1[3].y * bb.height() + y, color);
            drawList.addQuadFilled(QUAD_2[0].x * bb.width() + x, QUAD_2[0].y * bb.height() + y,
                QUAD_2[1].x * bb.width() + x, QUAD_2[1].y * bb.height() + y,
                QUAD_2[2].x * bb.width() + x, QUAD_2[2].y * bb.height() + y,
                QUAD_2[3].x * bb.width() + x, QUAD_2[3].y * bb.height() + y, color);
        }
        case CROSS -> {
