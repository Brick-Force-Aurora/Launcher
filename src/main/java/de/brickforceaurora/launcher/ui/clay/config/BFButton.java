...
public record BFButton(float paddingTopLeft, float shadowSize, float rounding, SimpleColor color, SimpleColor shadow) implements IElementConfig {
    ...
    public void buildOpenCommands(ElementContext context, Element element, IElementConfig elementConfig) {
        ...
        context.push(new RenderCommand("rectangle", element, new BoundingBox(bb.x() + paddingTopLeft, bb.y() + paddingTopLeft,
            bb.width() - shadowSize, bb.height() - shadowSize), Rectangle.filled(color, rounding)));
    }
