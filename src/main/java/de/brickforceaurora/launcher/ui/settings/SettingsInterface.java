...
import static de.brickforceaurora.launcher.ui.UserInterface.*;
...
public final class SettingsInterface {
    ...
    public SettingsInterface(RenderContext renderContext) {
    ...
    private void checkbox(String name, BooleanSupplier getter, BooleanConsumer setter) {
    ...
    public void updateState(LayoutContext layout, float deltaTime) {}

    public void createLayout(LayoutContext layout, float deltaTime) {
        ...
        builder.layout().childGap(4).layoutDirection(LayoutDirection.TOP_TO_BOTTOM).padding(NO_PADDING).width(ISizing.fixed(layout.width()))
            ...
            .padding(Padding.builder().left((int) (layout.width() * 0.6f)).top(40 + 8)
                .right(8).bottom(80 + 32 + 8).build());
